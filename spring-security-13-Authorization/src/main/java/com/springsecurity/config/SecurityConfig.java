...
public class SecurityConfig {
	...
	void permitAll(AuthorizeHttpRequestsConfigurer<HttpSecurity>
		.AuthorizationManagerRequestMatcherRegistry request) {
		request.anyRequest()
			.permitAll();
	}
	
	void denyAll(AuthorizeHttpRequestsConfigurer<HttpSecurity>
	.AuthorizationManagerRequestMatcherRegistry request) {
		request.anyRequest()
		.denyAll();
	}
	...
	void access2(AuthorizeHttpRequestsConfigurer<HttpSecurity>
		.AuthorizationManagerRequestMatcherRegistry request) {
		request.requestMatchers(AntPathRequestMatcher.antMatcher("/email/{email}"))
			.access((authentication, requestAuthorizationContext) -> {
				...
				String header = requestAuthorizationContext.getRequest().getHeader("trust");
